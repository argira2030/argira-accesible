...
(function() {
  ...
  async function startListen() {
    ...
    // 7. Completion gate — único punto de salida del flujo
    // Si stopListen() se ejecutó durante el await, _isListening es false: salir sin tocar nada.
    if (!_isListening) return;
    if (audio) playAudioFile(audio);
    else _resetListenUI();
  }
  ...
  function playAudioFile(src) {
    try {
      ...
      _currentAudio.onended = () => { _currentAudio = null; _isListening = false; _resetListenUI(); };
      _currentAudio.onerror = () => { _currentAudio = null; _isListening = false; _resetListenUI(); };
      _currentAudio.play().catch(() => { _isListening = false; _resetListenUI(); });
    } catch (e) { _isListening = false; _resetListenUI(); }
  }
