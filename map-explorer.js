...
(function() {
  ...
  // ------------------------------------------------------------------
    ...
    'VanGogh_Trigal_con_cipreses': {
      titulo: 'Trigal con cipreses',
      artista: 'Vincent van Gogh',
      descripcion: 'Obra de Vincent van Gogh. Explora su sonificación en el mapa perceptual.',
      img: 'VanGogh_Trigal_con_cipreses.jpg',
    },
    'Rafael_Adan_y_Eva': {
      titulo: 'Adán y Eva',
  ...
  let zoomOverlay, zoomContent, zoomImg, zoomCloseBtn, zoomBtn, zoomDynamicHost;
  ...
  let _zoomTouchHandle = null;  // { destroy() } del touch de imagen (argira-zoom-explore.js), null si no hay uno activo

  // "Leer descripción" del modal de zoom: repite el mismo texto que el
  // botón "Escuchar" del modal principal, pero SOLO narra (ArgiraSpeech.
  // speak) — sin playAnchor/sonificación WebAudio. Token de sesión propio
  // (_zoomAudioToken) para el guard de carrera del finally, igual patrón
  // que _audioToken del modal principal pero en namespace separado.
  let zoomListenBtn;
  let _zoomIsListening = false;
  let _zoomAudioToken  = 0; // invalida el finally de una sesión de speak() anterior

  // "Reproducir" del modal de zoom (Paso 3): NO es una sesión propia — es
  // el mismo botón lógico que #argira-obra-listen-btn, repetido con su
  // propia UI. Llama a toggleListen() (autoridad única de _isListening/
  // _audioToken en el modal principal) y se sincroniza reactivamente vía
  // el evento 'argira:listening-change' ya existente — no duplica estado.
  let zoomPlayBtn;
  ...
  function buildZoom() {
    ...
    div.innerHTML = `
      <div id="argira-zoom-content" role="dialog" aria-modal="true" aria-label="Imagen ampliada" tabindex="-1">
        <div id="argira-zoom-main">
          <img id="argira-zoom-img" src="" alt="" />
          <div id="argira-zoom-actions">
            <button id="argira-zoom-close-btn" class="argira-modal-btn" type="button" aria-label="Cerrar imagen ampliada">✕ Cerrar</button>
            <button id="argira-zoom-play-btn" class="argira-modal-btn" type="button" aria-pressed="false" hidden>🔊 Escuchar</button>
            <button id="argira-zoom-listen-btn" class="argira-modal-btn" type="button" aria-pressed="false" hidden>🔊 Leer descripción</button>
          </div>
        </div>
        <!-- Dynamic host contract:
             All temporary zoom UI (Touch, Tour, future overlays)
             must mount inside #argira-zoom-dynamic-host.
             Modules must never append children directly to
             #argira-zoom-content or #argira-zoom-main. -->
        <div id="argira-zoom-dynamic-host"></div>
      </div>
    `;
  ...
  // _resetZoomListenUI — solo UI del botón "Leer descripción" del zoom,
  // mismo patrón que _resetListenUI() del modal principal (bloque 1B).
  function _resetZoomListenUI() {
    if (!zoomListenBtn) return;
    zoomListenBtn.setAttribute('aria-pressed', 'false');
    zoomListenBtn.classList.remove('speaking');
    zoomListenBtn.innerHTML = '🔊 Leer descripción';
  }

  // stopZoomListen — corta la narración en curso, si la hay. Se llama al
  // cerrar Zoom (closeZoom) y al alternar el botón mientras está hablando.
  function stopZoomListen() {
    _zoomIsListening = false;
    _zoomAudioToken++;
    if (window.ArgiraSpeech) window.ArgiraSpeech.stop();
    _resetZoomListenUI();
  }

  // startZoomListen — narra el mismo texto que compone el botón "Escuchar"
  // del modal principal (título, artista, descripción), leído de los mismos
  // dataset del listenBtn ya poblados en openModal(). Solo ArgiraSpeech: sin
  // playAnchor/sonificación WebAudio, que es responsabilidad del modal
  // principal — decisión ya tomada, ver ROADMAP.md ("pieza más simple").
  // Mismo contrato que startListen(): ArgiraSpeech.speak() devuelve una
  // Promise que resuelve al terminar la locución (no un callback onEnd) —
  // se usa await + token de sesión propio, igual patrón que el modal
  // principal (bloque 1E-quinquies), para que un cierre/reapertura rápida
  // no deje una sesión vieja pisando el estado de la nueva.
  async function startZoomListen() {
    if (_zoomIsListening) return;
    if (!window.ArgiraSpeech) return;
    ...
    const token = ++_zoomAudioToken;

    _zoomIsListening = true;
    zoomListenBtn.setAttribute('aria-pressed', 'true');
    zoomListenBtn.classList.add('speaking');
    zoomListenBtn.innerHTML = '⏹ Detener';

    try {
      await window.ArgiraSpeech.speak(textoCompleto, { rate: 0.92 });
    } finally {
      // Guard de carrera: si stopZoomListen() ya corrió (token cambió,
      // p.ej. el usuario pulsó Detener o cerró el modal), no repetir
      // el trabajo ni pisar un estado que ya quedó correcto.
      if (token === _zoomAudioToken) {
        _zoomIsListening = false;
        _resetZoomListenUI();
      }
    }
  ...
  function toggleZoomListen() {
    if (_zoomIsListening) stopZoomListen();
    else startZoomListen();
  }

  // _syncZoomPlayUI — refleja _isListening (autoridad única, ver bloque 1B)
  // en zoomPlayBtn. NO escribe _isListening ni llama a start/stopListen: es
  // puramente reactiva, igual que el patrón ya usado por index.html y las
  // capas C2/C4 para 'argira:listening-change'. _resetListenUI() sigue
  // siendo el único punto que muta listenBtn; esta función solo copia su
  // resultado visual a zoomPlayBtn cuando ambos botones coexisten.
  function _syncZoomPlayUI() {
    if (!zoomPlayBtn) return;
    zoomPlayBtn.setAttribute('aria-pressed', _isListening ? 'true' : 'false');
    zoomPlayBtn.classList.toggle('speaking', _isListening);
    zoomPlayBtn.innerHTML = _isListening ? '⏹ Detener' : '🔊 Escuchar';
  }

  // Suscripción única a nivel de módulo (no dentro de openZoom, para no
  // acumular listeners en aperturas repetidas). Solo sincroniza UI; el
  // evento ya existía y ya lo consumen index.html + argira-harmonic-blend.js
  // + argira-region-breathing.js + argira-region-impulse.js — este listener
  // no altera su emisión ni su payload, solo añade un consumidor más.
  window.addEventListener('argira:listening-change', _syncZoomPlayUI);
  ...
  function openZoom(srcUrl, altText, triggerBtn) {
    ...
    if (!zoomOverlay) {
      ...
      zoomPlayBtn   = document.getElementById('argira-zoom-play-btn');
      zoomListenBtn = document.getElementById('argira-zoom-listen-btn');
      // Dynamic host contract (ver buildZoom()): único punto de montaje
      // para UI temporal del zoom (Touch, Tour, futuros overlays).
      // Ningún módulo dinámico debe recibir zoomContent ni zoomMain.
      zoomDynamicHost = document.getElementById('argira-zoom-dynamic-host');

      zoomCloseBtn.addEventListener('click', closeZoom);
      // toggleListen() es la MISMA función que usa #argira-obra-listen-btn
      // (línea ~1188) — no una copia. Un solo botón lógico, dos disparadores.
      zoomPlayBtn?.addEventListener('click', toggleListen);
      zoomListenBtn?.addEventListener('click', toggleZoomListen);
    ...
    // "Reproducir": visible según el MISMO criterio que listenBtn.disabled
    // en openModal() (línea ~1237) — hay obra en CATALOGUE o descripción.
    // La UI (aria-pressed/clase/texto) NO se fija aquí: la sincroniza el
    // listener de 'argira:listening-change' de más abajo, que es la misma
    // fuente que ya usan index.html y las capas C2/C4 — así zoomPlayBtn
    // arranca ya en el estado correcto si _isListening ya era true al
    // abrir Zoom (p. ej. el usuario pulsó Escuchar y luego abrió Zoom).
    if (zoomPlayBtn) {
      zoomPlayBtn.hidden = !_currentObra && !(listenBtn?.dataset.texto);
      _syncZoomPlayUI();
    }

    // "Leer descripción": visible solo si hay texto que narrar — mismo
    // criterio que listenBtn.disabled en openModal() (línea ~1137).
    // hidden en vez de disabled porque, a diferencia del modal principal,
    // el zoom puede abrirse repetidas veces sin volver a pasar por
    // openModal(); ocultar es más claro que un botón deshabilitado
    // "de fábrica" cada vez que no hay descripción.
    if (zoomListenBtn) {
      const hayTexto = !!(listenBtn?.dataset.texto || listenBtn?.dataset.titulo);
      zoomListenBtn.hidden = !hayTexto;
    }

    // Touch de imagen (Paso 1, ver plan-tour-touch-integracion.md): se
    // inicializa una vez que la imagen ampliada terminó de cargar, para
    // no leer píxeles de un <img> todavía vacío. Si ya estaba completa
    // (caché de navegador), .complete es true de inmediato y el listener
    // 'load' nunca dispararía, por eso se comprueba antes de suscribirse.
    // Monta en zoomDynamicHost, no en zoomContent (dynamic host contract,
    // ver buildZoom()) — así el touch nunca compite por layout con
    // #argira-zoom-main.
    if (_zoomTouchHandle) { _zoomTouchHandle.destroy(); _zoomTouchHandle = null; }
    if (window.initZoomTouch) {
      const startTouch = () => {
        if (!_zoomOpen) return; // el modal ya se cerró mientras cargaba
        _zoomTouchHandle = window.initZoomTouch(zoomDynamicHost, zoomImg);
      };
      if (zoomImg.complete && zoomImg.naturalWidth > 0) {
        startTouch();
      } else {
        zoomImg.addEventListener('load', startTouch, { once: true });
      }
    }
  ...
  function closeZoom() {
    ...
    // Cortar "Leer descripción" si estaba narrando — mismo criterio que el
    // touch de imagen justo debajo: nada debe seguir sonando tras cerrar.
    if (_zoomIsListening) stopZoomListen();

    // Cortar la exploración táctil: quita listeners del canvas y su DOM,
    // y su flag `destroyed` interno evita que un speak() diferido (el
    // setTimeout de 500ms en handleTouch) hable después de cerrado.
    if (_zoomTouchHandle) { _zoomTouchHandle.destroy(); _zoomTouchHandle = null; }
  ...
  //
  // REDISEÑO (bloque 1E-quinquies): antes, _isListening permanecía en true
  // indefinidamente tras terminar playAnchor()+speech — el comentario
  // original decía literalmente "el botón en Detener [...] _isListening
  // permanece true" — así que el ambiente (C1-C4) solo se restauraba si el
  // usuario pulsaba Detener a mano. Ahora la sesión de "Escuchar" es una
  // operación con principio y fin definidos por la propia reproducción:
  // termina sola en cuanto playAnchor()+speech completan, sin esperar
  // acción del usuario. El try/finally garantiza que _isListening vuelve
  // a false y el evento 'listening:false' se emite incluso si playAnchor()
  // lanza una excepción o la sesión es cancelada a mitad de camino.
  //
  // stopListen() sigue existiendo para el caso en que el usuario quiera
  // interrumpir ANTES de que termine sola — pulsar Detener mientras suena.
  // Guard de carrera: el 'finally' comprueba el token de sesión (capturado
  // al principio) antes de tocar _isListening/UI/evento — si stopListen()
  // ya corrió primero (usuario pulsó Detener), el token ya no coincide y el
  // finally no debe pisar el estado que stopListen() ya dejó correcto.
  async function startListen() {
    ...
    // 2. Nueva sesión — token invalida callbacks async de la sesión anterior
    //    y sirve de guard de carrera para el finally (ver comentario arriba).
    const token = ++_audioToken;

    // 3. Fuente de verdad JS primero — no el DOM
    _isListening = true;
    listenBtn.setAttribute('aria-pressed', 'true');
    listenBtn.classList.add('speaking');
    listenBtn.innerHTML = '⏹ Detener';

    // Observabilidad (bloque 1E): CustomEvent, no controla estado — index.html
    // y las capas C2/C4 lo escuchan para silenciar el ambiente mientras suena
    // la sonificación de la obra.
    window.dispatchEvent(new CustomEvent('argira:listening-change', { detail: { listening: true } }));

    // 4. Capturar estado antes del primer await (evita stale closure tras suspensión)
    ...
    try {
      // 5. WebAudio: pipeline primario — campo sonoro activo antes que la voz
      ...
      // 6. Speech: awaited — paso en el flujo, no controlador del estado
      const speechPromise = window.ArgiraSpeech
        ? window.ArgiraSpeech.speak(textoCompleto, { rate: 0.92 })
        : Promise.resolve();
      await speechPromise;
    } finally {
      // 7. Cierre automático de la sesión — este es el cambio de fondo:
      //    antes este bloque no existía y la sesión quedaba abierta hasta
      //    que el usuario pulsara Detener. Ahora playAnchor+speech son el
      //    ciclo de vida completo de "Escuchar".
      //    Guard de carrera: si stopListen() ya corrió (token cambió),
      //    _isListening y la UI ya están en su estado correcto — no repetir
      //    el trabajo ni volver a emitir el evento por duplicado.
      if (token === _audioToken) {
        _isListening = false;
        _resetListenUI();
        window.dispatchEvent(new CustomEvent('argira:listening-change', { detail: { listening: false } }));
      }
    }
  }
  ...
  // SESSION EXIT POINT — único lugar donde _isListening pasa a false por
  // intervención del usuario (interrumpir "Escuchar" antes de que termine
  // sola). El cierre automático al terminar playAnchor+speech vive en el
  // finally de startListen() (ver bloque 1E-quinquies); este stopListen()
  // sigue siendo necesario para el caso "el usuario pulsa Detener a mitad
  // de la sonificación". _audioToken++ es lo que hace que el finally de la
  // sesión en curso detecte la cancelación (token !== _audioToken) y no
  // repita el trabajo que stopListen() ya hizo aquí.
  function stopListen() {
    ...
    _stopAllAudio();
    _resetListenUI();
    // Observabilidad (bloque 1E): simétrico al dispatch de startListen —
    // index.html y las capas C2/C4 restauran el ambiente al dejar de sonar.
    window.dispatchEvent(new CustomEvent('argira:listening-change', { detail: { listening: false } }));
  }
