// ================================================================
//  ARGIRA · MAP EXPLORER · FASE 2D (Pointer Model Unificado)
//  ================================================================
//  Interacción limpia basada en pointerup + máquina de estados.
//  Elimina touchstart/touchend/dblclick/timers manuales.
//  Compatible con setAnchorFromHover del index.html.
// ================================================================
...
(function() {
  ...
  // ------------------------------------------------------------------
  // 3. UTILIDADES DE COORDENADAS (compatibles con el canvas del index)
  // ------------------------------------------------------------------
  function getP1Range() {
    if (typeof window.P1_MIN !== 'undefined') return { min: window.P1_MIN, max: window.P1_MAX };
    return { min: 1.0, max: 5.0 }; // fallback
  }
  function getP4Range() {
  ...
  // ------------------------------------------------------------------
  // 4. OVERLAY Y NODOS DOM (FASE 2A, 2B, 2C)
  // ------------------------------------------------------------------
  function initOverlayAndNodes() {
    ...
    function renderMapNodes() {
      ...
      overlayDiv.querySelectorAll('.map-node').forEach(n => n.remove());
      window.CATALOGUE.forEach(obra => {
  ...
  // ------------------------------------------------------------------
  // 5. INTERACCIÓN UNIFICADA CON POINTER EVENTS (FASE 2D)
  // ------------------------------------------------------------------
  ...
  function handlePointerUp(e) {
    ...
    if (isDouble) {
      openModal(meta);
      lastPointer.time = 0; // reset para no encadenar
      return;
    }
  ...
  // ------------------------------------------------------------------
  // 6. ACTIVACIÓN DE NODOS POR TECLADO (FASE 2C)
  // ------------------------------------------------------------------
  function initNodeKeyboard() {
    const overlayDiv = document.getElementById('map-overlay');
    if (!overlayDiv) return;
    overlayDiv.addEventListener('click', (e) => {
      const node = e.target.closest('.map-node');
      if (!node) return;
      const obra = window.CATALOGUE?.find(o => o.id === node.dataset.id);
      if (obra) openModal(findMeta(obra.id));
    });
    overlayDiv.addEventListener('keydown', (e) => {
  ...
  // ------------------------------------------------------------------
  // 7. INICIALIZACIÓN PRINCIPAL (con reintentos)
  // ------------------------------------------------------------------
  function init() {
    if (!document.getElementById('map') || !window.CATALOGUE || !window.CATALOGUE.length) {
      console.warn('ARGIRA explorer: esperando canvas y catálogo...');
      setTimeout(init, 200);
      return;
    }
    initOverlayAndNodes();
    ...
    initNodeKeyboard();
    console.log('ARGIRA FASE 2D activa — interacción unificada por pointerup');
  }
