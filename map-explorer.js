...
(function() {
  ...
  function initOverlayAndNodes() {
    ...
    function renderMapNodes() {
      ...
      // Permite al usuario de TalkBack saltar a la siguiente zona sin
      // recorrer todas las obras de la actual. El último bloque ofrece
      // "Salir del mapa" en lugar de "siguiente zona".
      // El botón es off-screen (mismo patrón que _makeExitNode) para no
      // contaminar la UI visual.
      activeSections.forEach(({ section, label }, i) => {
        ...
        if (isLast) {
          skipBtn.setAttribute('aria-label', 'Fin de las zonas. Salir del mapa');
          skipBtn.addEventListener('click', () => {
            const target = document.getElementById('btn-audio-header');
            if (target) target.focus();
          });
        } else {
          const nextLabel = activeSections[i + 1].label;
          skipBtn.setAttribute('aria-label',
            `Saltar a siguiente zona: ${nextLabel}`);
          skipBtn.addEventListener('click', () => {
            // Mover el foco al primer map-node de la siguiente section
            const nextSection = activeSections[i + 1].section;
            const firstNode = nextSection.querySelector('.map-node');
            if (firstNode) {
              // Activar roving tabindex en ese nodo antes de enfocar
        ...
        skipBtn.style.cssText =
          'position:absolute;left:-9999px;width:1px;height:1px;overflow:hidden;white-space:nowrap;';
