 ...
 *
 * AUDIO LAYERS
 * 1. WebAudio  (continuous perceptual field)
 *    — started in startListen(), stopped only in stopListen()
 *    — never controlled by WAV or speech
 *
 * 2. WAV       (discrete perceptual event)
 *    — NEVER modifies _isListening, UI, or WebAudio
 *    — emits argira:wav-ended / argira:wav-error only
 *
 * 3. Speech    (semantic layer)
 *    — independent of WAV and WebAudio
...
(function() {
  ...
  async function startListen() {
    ...
    // Sin .wav: el campo sonoro (WebAudio) continúa activo hasta que el usuario pulse stop.
    // No se llama _resetListenUI() — _isListening permanece true y el botón en "⏹ Detener".
  }

  // DISCRETE PERCEPTUAL LAYER (WAV) — no puede modificar estado ni UI principal.
  function playAudioFile(src, token) {
    ...
    try {
      ...
      _currentAudio.onended = () => {
        ...
        _currentAudio = null;
        // El campo sonoro (WebAudio) sigue activo y el botón permanece en "⏹ Detener".
        // Solo stopListen() (vía toggleListen o closeModal) termina la sesión.
        window.dispatchEvent(new CustomEvent('argira:wav-ended', { detail: { src } }));
      };
      _currentAudio.onerror = () => {
        if (token !== _audioToken) return;
        _currentAudio = null;
        // Igual que onended: no resetear UI. El WebAudio sigue corriendo.
        window.dispatchEvent(new CustomEvent('argira:wav-error', { detail: { src } }));
      };
      _currentAudio.play().catch(() => {
        if (token !== _audioToken) return;
        // play() rechazado: el .wav no arrancó, pero el WebAudio ya está corriendo.
        // No reseteamos UI — el usuario puede detener manualmente.
        window.dispatchEvent(new CustomEvent('argira:wav-error', { detail: { src } }));
      });
    } catch (e) {
      if (token !== _audioToken) return;
      // Excepción al crear Audio: ídem, no resetear UI.
    }
  }
  ...
  function _stopAllAudio() {
    ...
    //    CRÍTICO: handlers a null ANTES de mutar src/load.
    //    Firefox puede disparar onerror síncronamente en load() si src="" es inválido.
    //    Si los handlers siguen vivos en ese momento, corren con el token aún válido
    //    (cuando _stopAllAudio se llama desde paths distintos a stopListen).
    //    Null-out primero = ningún callback puede tocar estado después de este punto.
    if (_currentAudio) {
