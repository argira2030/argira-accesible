  ...
  function initOverlayAndNodes() {
    ...
    function renderMapNodes() {
      ...
      activeSections.forEach(({ section, label }, i) => {
        ...
        // Botón 1: saltar a siguiente zona (todas excepto la última)
        if (!isLast) {
          ...
          skipBtn.setAttribute('aria-label',
            `Zona ${pos} de ${total}. Saltar a siguiente zona: ${nextLabel}`);
        ...
        // Botón 2: salir del mapa — presente en TODAS las zonas
        const exitBtn = _makeZoneExitBtn();
        exitBtn.setAttribute('aria-label',
          `Fin de zona ${pos} de ${total}. Salir del mapa. Volver al menú principal`);
