...
(function() {
  ...
  function initOverlayAndNodes() {
    ...
    // Cuando mapWrap recibe foco (Tab desde header), activa el primer nodo
    const mapWrapEl = document.getElementById('mapWrap');
    if (mapWrapEl) {
      mapWrapEl.addEventListener('focus', () => {
        const nodes = _getNodes();
        if (!nodes.length) return;
        // Ordenar por P1 y activar el primero (izquierda del mapa)
        const sorted = _sortedByP1(nodes);
        const firstIdx = _getNodes().indexOf(sorted[0]);
        _rovingFocus(firstIdx >= 0 ? firstIdx : 0);
      });
