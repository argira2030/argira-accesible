...
(function() {
  ...
  function openModal(meta) {
    ...
    if (nivelEl) {
      nivelEl.textContent = '';
      nivelEl.setAttribute('aria-hidden', 'true');
    }
    ...
    // Estrategia de foco al cerrar (por prioridad):
    // 1. El map-node button de esta obra en el overlay — elemento semánticamente correcto.
    // 2. document.activeElement si es enfocable y no es body ni mapWrap.
    // 3. #btn-ayuda como último recurso fuera del mapa.
    // document.activeElement en doble tap desde TalkBack suele ser body o mapWrap,
    // que no tienen contenido anunciable — de ahí que el foco "se pierda".
    ...
    // inert en #app: bloquea TalkBack fuera del modal (footer, header, mapa)
  ...
  function initOverlayAndNodes() {
    ...
    function renderMapNodes() {
      ...
      window.CATALOGUE.forEach(obra => {
        ...
        btn.setAttribute('tabindex', '-1'); // roving tabindex — el foco entra por mapWrap
        btn.dataset.id = obra.id;
        const meta = findMeta(obra.id);
        btn.setAttribute('aria-label', `${meta.titulo}, de ${meta.artista}. Pulsa Enter para abrir`);
        const cx = PAD + normP1(obra.P1) * (W - PAD * 2);
        const cy = PAD + (1 - normP4(obra.P4)) * (H - PAD * 2);
      ...
      mapWrapEl.addEventListener('keydown', e => {
        ...
        if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
          ...
          if (meta && obra) {
            ...
            const p1min = window.P1_MIN, p1max = window.P1_MAX;
            const p4min = window.P4_MIN, p4max = window.P4_MAX;
            const zonaH = obra.P1 < (p1min + (p1max - p1min) * 0.33) ? 'zona izquierda' :
                          obra.P1 < (p1min + (p1max - p1min) * 0.66) ? 'zona central' :
                          'zona derecha';
            const zonaV = obra.P4 < (p4min + (p4max - p4min) * 0.33) ? ', parte inferior' :
                          obra.P4 < (p4min + (p4max - p4min) * 0.66) ? ', parte media' :
                          ', parte superior';
            next.setAttribute('aria-label',
              `${meta.titulo}, ${meta.artista}. ${zonaH}${zonaV}. Obra ${pos} de ${total}. Pulsa Enter para abrir.`
            );
          }
        }
        ...
        if (e.key === 'Tab' || e.key === 'Escape') {
          ...
          if (e.key === 'Escape') {
            e.preventDefault();
            // Pausar audio del campo perceptual al salir con Escape
            if (window._argiraAudioCtx && window._argiraAudioCtx.state === 'running') {
              window._argiraAudioCtx.suspend().then(() => {
                if (typeof window._stopFieldOsc === 'function') window._stopFieldOsc();
                const btnAudio = document.getElementById('btn-audio-header');
  ...
  function initPointerInteraction() {
    ...
    function handlePointerUp(e) {
      ...
      if (isDouble) {
        // Cancelar el anuncio SR del primer tap: clearTimeout cancela el setTimeout(50)
        // pendiente en window._anchorAnnounceTid antes de que escriba en #sr-announcer.
        // Solo vaciar el nodo no sería suficiente — el callback seguiría ejecutándose.
      ...
      // Un solo toque → mostrar obra en panel sin reproducir audio.
      // El arrastre sigue produciendo sonido espacial; el tap es para identificar/comparar.
      // El doble tap abre el modal con controles completos.
      ...
      if (elDominantTitle && meta) {
        elDominantTitle.textContent = `${meta.titulo} · ${meta.artista}`;
      }

      // Calcular distancia y peso del nodo más cercano para mostrar en panel
      if (typeof window.clientToNearest === 'function' || obra) {
        const result = window.lastResult;
        if (result && result.neighbours && result.neighbours[0]) {
          const nb = result.neighbours[0];
          if (elDWeight) elDWeight.textContent = (nb.w * 100).toFixed(0) + '%';
          if (elDDist)   elDDist.textContent   = nb.d.toFixed(3);
        }
      }

      // Pista visual la primera vez: "Doble toque para abrir"
      if (!sessionStorage.getItem('argira:dblTapHint')) {
        const hint = document.getElementById('dominantTitle');
        if (hint) {
          const prev = hint.textContent;
          hint.textContent = `${prev}  ·  Doble toque para abrir`;
          setTimeout(() => { hint.textContent = prev; }, 2200);
          sessionStorage.setItem('argira:dblTapHint', '1');
        }
      }
      ...
      if (srAnnouncer && meta) {
        ...
        clearTimeout(window._anchorAnnounceTid);
        window._anchorAnnounceTid = setTimeout(() => {
          srAnnouncer.textContent = `${meta.titulo}, de ${meta.artista}. Doble toque para abrir.`;
        }, 50);
      }
