...
(function () {
  ...
  // FIX (bloque 1E-quater): C4 es "continuo autónomo" por diseño — una vez
  // arrancado (_start), sigue sonando indefinidamente hasta el PRÓXIMO
  // argira:region-change, sin relación alguna con el ciclo de vida del
  // modal/Escuchar. Al pulsar "Escuchar" en una obra, si ya había un
  // carrier de Capa 4 sonando desde antes, seguía audible y se solapaba
  // con la sonificación — este módulo nunca escuchaba el estado de sesión
  // de escucha porque no existía ese evento cuando se escribió C4.
  // Aquí no rompemos su contrato ("C4 no toca C2 ni C3"): solo silenciamos/
  // restauramos su propia ganancia en respuesta a argira:listening-change,
  // igual que ya hace fieldGain (C1) en index.html.
  window.addEventListener('argira:listening-change', (e) => {
    const listening = !!e.detail?.listening;
    const ctx = window.audioCtx;
    if (!_gainNode || !ctx) return;
    const now = ctx.currentTime;
    if (listening) {
      _gainNode.gain.setTargetAtTime(0, now, 0.08);
    } else {
      _gainNode.gain.setTargetAtTime(GAIN_BASE, now, 0.3);
    }
  });
