...
(function () {
  ...
  // FIX (bloque 1E-quater): mismo criterio que C4 (RegionBreathing) — C2
  // también es un oscilador propio conectado directo a audioMasterGain,
  // sin relación con el ciclo de vida del modal/Escuchar. Silenciamos su
  // ganancia (no _stop() completo, para no perder el estado _active/_osc
  // y poder retomar sin reiniciar) mientras se escucha una obra.
  let _blendListeningGain = null;
  window.addEventListener('argira:listening-change', (e) => {
    ...
    if (listening) {
      _gain.gain.setTargetAtTime(0, now, 0.08);
    } else if (_active) {
      _gain.gain.setTargetAtTime(GAIN_BASE, now, 0.3);
    }
  });
